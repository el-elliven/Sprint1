...
def purchase_item():
    ...
    # check if id in product exists
        ...
        if customer_exists and product_exists:
            ...
            amount_bought = purchase_item_quantity
            space_small
            for i in range(len(data_product)):
                if product_id_from_user in data_product[i][0]:
                    ...
                    if purchase_item_quantity > quantity:
                        print(name + " can't be sold because the quantity in stock is Lower than ordered " + str(
        ...
        item = Purchase(customer_id, product_id, amount_bought)
        with open("purchase.csv", "a+", newline="") as fp:
            wr = csv.writer(fp, dialect='excel')
            wr.writerow(item.__str__())

def update_stock():
    global new_prod_quantity
        ...
        while (s):
            ...
            if len(s) > 0:
                if (L[0]) == product_ID:
                    ...
                    temp.write(
                        str(product_ID) + ',' + product_name + ',' + str(new_prod_quantity) + ',' + str(product_price))
                else:
                    temp.write(s)
        temp.close()
        file.close()
